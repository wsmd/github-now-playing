...
import { ITunesSourceProvider } from 'lib/sources/itunes/ITunesSourceProvider';

beforeEach(jest.useFakeTimers);
afterEach(jest.clearAllTimers);

describe('ITunesSourceProvider', () => {
  it('calls osascript.jxa with correct script', () => {
    ...
    const source = new ITunesSourceProvider({ updateFrequency: 10000 });
    source.listen();
  ...
  it('reports the track via osascript', async () => {
    ...
    const source = new ITunesSourceProvider({ updateFrequency: 1 });
    source.on(ITunesSourceProvider.Events.TrackChanged, onChangeFn);
    source.listen();
